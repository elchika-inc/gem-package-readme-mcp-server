...
export function handleApiError(error: unknown, context: string): never {
  ...
  if (error instanceof Error) {
    ...
    throw new GemReadmeMcpError(`Unexpected error in ${context}`, 'UNEXPECTED_ERROR', undefined, error);
  }

  logger.error(`Unknown error in ${context}`, { error });
  throw new GemReadmeMcpError(`Unknown error in ${context}`, 'UNKNOWN_ERROR', undefined, error);
}
...
export function handleHttpError(status: number, response: Response, context: string): never {
  ...
  switch (status) {
    case 404:
      throw new GemNotFoundError(context);
    case 429: {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error instanceof NetworkError || 
          (error instanceof GemReadmeMcpError && error.statusCode && error.statusCode >= 500)) {
